...
export async function GET(req: NextRequest) {
    ...
    const fromDate = sort === 'desc'
        ? refDate.subtract(size, 'day').format('YYYY-MM-DD')
        : refDate.format('YYYY-MM-DD');
    const toDate = sort === 'desc'
        ? refDate.format('YYYY-MM-DD')
        : refDate.add(size, 'day').format('YYYY-MM-DD');
    ...
    const [{ data: oldestRow }, { data: latestRow }] = await Promise.all([
        supabase.from('results').select('draw_date').order('draw_date', { ascending: true }).limit(1).single(),
        supabase.from('results').select('draw_date').order('draw_date', { ascending: false }).limit(1).single(),
    ]);
